...
" most Scala keywords
syn keyword GMKeyword nodeProperty edgeProperty N_P E_P  nodeProp edgeProp
syn keyword GMKeyword for foreach if else proc procedure do while return local
syn keyword GMKeyword inNbrs outNbrs nbrs nodes edges commonNbrs nbrEdges
...
syn keyword GMBuiltin log rand pow exp uniform sqrt
syn keyword GMBuiltin pushBack popBack pushFront popFront has add remove addAll retainOnly removeAll isSubsetOf clear
...
syn keyword GMKeyword sum product max hin count all exist avg
syn keyword GMKeyword require
syn keyword GMType int float double bool graph long node edge dGraph uGraph
syn keyword GMType nodeSet N_S nodeOrder N_O nodeSequence nodeSeq N_Q 
syn keyword GMType map collection string
syn keyword GMOP and
syn keyword GMOP or
syn keyword GMNumber NIL
syn match GMOP "+="
syn match GMOP "&&"
syn match GMOP "||"
syn match GMOP "min="
...
"TODO end \n or not?
syn region GMString start="\"[^"]" skip="\\\"" end="\"" contains=GMStringEscape 
syn match GMStringEscape "\\u[0-9a-fA-F]\{4}" contained
syn match GMStringEscape "\\[nrfvb\\\"]" contained

" symbol and character literals
syn match GMSymbol "'[a-zA-Z0-9][_a-zA-Z0-9]*\>"
syn match GMType "[$][a-zA-Z0-9][_a-zA-Z0-9]*\>"
